(ns shoes.rules
  ...
  (:require [precept.rules :refer [rule session defsub define]]
            ...
            [shoes.schema :refer [db-schema client-schema]]
            [shoes.api :as api]))
...
(rule print-visible-products [[:app :visible-product/id ?e]]
                           [[?e :product/name ?name]]
						   [[?e :product/price ?price]]
						   [[?e :product/category ?cat]]
  => (.log js/console (str "Category: " ?cat " Name: " (apply str (take 5 ?name))  " Price: " ?price) ))
...
(rule drill-down
      [[:app :visible-product/id ?e]]
      [[_ :product-filter/category ?selected-category]]
      [[?e :product/category ?selected-category]]
      [[_ :product-filter/category ?selected-category]]
      [[?e :product/category ?selected-category]]
      => (.log js/console (str "Drilled Down product: " ?e)))
;; Sorting
...
(session app-session 'shoes.rules
  :db-schema db-schema
