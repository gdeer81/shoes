...
(defn intro []
      [:div {:class "foo"}
       ...
       [:p
        [:a {:class "btn btn-primary" :href "https://conarrative.github.io/precept/" :role "button"}
         "Precept Docs"]]])
...
(def filter-option-data
  {:range [{:label "$0 to $10"
            :facts {:db/id :product-filter
                    :filter-menu/selected "$0 to $10"
                    :product-filter/range [0 10]}}
		  {:label "$11 to $24"
            :facts {:db/id :product-filter
                    :filter-menu/selected "$11 to $24"
                    :product-filter/range [11 24]}}
           {:label "$25 to $50"
            :facts {:db/id :product-filter
	...
	:category [{:label "Casual"
			   :facts  
			   {:db/id :product-filter
				:filter-menu2/selected "Casual"
                :product-filter/category "Casual"}}
			  {:label "Dress"
			  :facts
			  {:db/id :product-filter
			   :filter-menu2/selected "Dress"
			   :product-filter/category "Dress"}}
			 {:label "Business"
			  :facts
			  {:db/id :product-filter
			   :filter-menu2/selected "Business"
			   :product-filter/category "Business"}}
			 {:label "Costume"
			  :facts
			  {:db/id :product-filter
			   :filter-menu2/selected "Costume"
			   :product-filter/category "Costume"}}]})
...
(defn filter-menu2 []
  (let [{:keys [selected]} @(subscribe [:filter-menu2])]
    [:div {:class "col-sm-1"}
     [:label "Cat Filter:"]
     [:div {:class "dropdown"}
      [:button {:class "btn btn-default dropdown-toggle" :type "button" :id "dropdownMenu10" :data-toggle "dropdown" :aria-haspopup "true" :aria-expanded "true"}
       (or selected "Cat Filter")
       [:span {:class "caret"}]]
      [:ul {:class "dropdown-menu" :aria-labelledby "dropdownMenu10"}
       [:li {:class "dropdown-header"} "Category"]
       (for [props (:category filter-option-data)]
         ^{:key (:label props)} [menu-option props])
	   [:li {:role "separator" :class "divider"}]
       [menu-option {:label "Clear"
                     :facts {:db/id :transient :remove-entity :product-filter}}]]]]))
...
(defn product [{:keys [db/id product/name product/category product/price product/img hovered?]}]
    [:div {:class "col-sm-6 col-md-4"
     ...
     [:div {:class "product-tile"}
      ...
      [:div {:class "row"}
       [:div {:class "col-xs-4"}
        [:img {:src (str "images/" img) :alt "..." :class "img-responsive product-image"}]]
       [:div {:class "col-xs-8"}
        [:p {:class "product-description"}]
        (str "Category: " category " Description: This is the product description. It can be a summary as this is just
        the product tile.")]]
      [:div {:class "row"}
